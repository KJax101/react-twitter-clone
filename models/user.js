// mongoose let's our server communicate to the mongoDB database
const mongoose = require('mongoose');
const Schema = mongoose.Schema;
const passportLocalMongoose = require('passport-local-mongoose');

const User = new Schema({
  /* 
    Define your user information fields for the schema here, 
    DO NOT MAKE "username" and "password" fields, since passport will set those for you

    IF nothing else is put in here, your User collection will 
    look like this automatically because of Passport...

    {
      "_id": ObjectId("5b3aeb21b8a16a4aa772836e"),
      "salt": "2ec720e0a978332f5cb9781341b456cd897a503e09eabd918359018a2b51957e",
      "hash": "",
      "username": "alex",
      "__v": 0
    }
  */
});

// Set up passport to 
User.plugin(passportLocalMongoose);

module.exports = mongoose.model('User', User);
